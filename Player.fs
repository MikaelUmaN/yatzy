...
let makeLegalPlayer (playerLogic: IPlayer) (allowedRepetitions: int) =
    { new IPlayer with
        override _.Name = playerLogic.Name

        override _.Act playerState dice nRemaining =
            let rec act failures =
                if failures > allowedRepetitions then
                    ...
                    try
                        let keptDice =
                            playerLogic.Act playerState dice nRemaining
                        ...
                        let hasKeptdice =
                            ...
                            |> List.forall
                                (fun (d, ds) ->
                                    let groupedCurrentdice = dice |> List.groupBy id |> Map.ofList

                                    groupedCurrentdice.ContainsKey d
                                    && groupedCurrentdice.[d].Length >= ds.Length)
        ...
        override _.SelectScore playerState dice =
...
let makeHumanTerminalPlayer (name: string) =
    { new IPlayer with
        ...
        override _.Act playerState dice nRemaining =
            ...
            let keepdice =
                try
                    List.map (int >> enum<DiceSide>) (Console.ReadLine().Split(' ') |> Array.toList)
                with _ -> []
